...
public class OgataLogLike_Calc {
  ...
  public void set_OmoriParms(double[] OmoriParms, ObsEqkRupList aftershockList) {
    ...
    //System.out.println("##aftershockList " +aftershockList.size());
    if(aftershockList.size() <= 1){
    	intEventTimes = new long[0];
    	minIntTime =0;
 	    maxIntTime = 0;
    }else{
    	intEventTimes = ObsEqkRupListCalc.getInterEventTimes(aftershockList);
	   // System.out.println("intEventTimes " + intEventTimes.length + "##aftershockList " + aftershockList.size() + " mag " + aftershockList.getObsEqkRuptureAt(0).getMag());
