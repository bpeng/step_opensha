...
public class UpdateSTEP_Forecast {
 ...
 // private boolean gridIsUpdated = false;
  ...
  private void initUpdate() {
	  ...
	  //TODO check????	  
	  backgroundRatesGrid = sequenceModel.getBackgroundRatesGrid().getRegion();
	  ...
	  //TODO check
	  if (sequenceModel.isGridIsUpdated()) {//this is done in checkTypeIIAftershockZones in STEP_main when a ASZ changes from a circle to sausage
		  updateGenericModelParms();
	  }else{//check and update update ASZ anyway if it is not updated in sequenceModel, this applies to the models that is already a sausage ASZ
//		  if(updateAftershockZone()){	//TODO this is commented to avoid the sausage ASZ to grow to big, 22/3/2010 Matt, for China catalog only?	 
//			  updateGenericModelParms();
//		  }
	  }
	  ...
	  /**
	   * update the list of distances from the fault
	   */
	  //UpdateGridDistFromFault();
   ...
   * Q. if it is already a sausage ASZ, is it necessary to update? A. yes
   */
  public boolean updateAftershockZone() {
	  return  sequenceModel.updateAftershockZone();
//	  int numAftershocks = aftershocks.size();
//	  //System.out.println(">> updateAftershockZone numAftershocks " + numAftershocks );	  
...
//	  } 
  ...
  public void updateGenericModelParms() {
	  ...
	  double[] kScaler = DistDecayFromRupCalc.getDensity(sequenceModel.getMainShock(),sequenceModel.getAfterShockZone());
		sequenceModel.setGridDistanceFromFault(DistDecayFromRupCalc.getRupDistList());
  ...
  public void updateSequenceModelParms() {
	  ...
	  System.out.println("Seq p-value = "+forecastModelSeq.get_pValSequence());
	  System.out.println("Seq b-value = "+forecastModelSeq.get_bValSequence());
	  System.out.println("Seq Mc = "+forecastModelSeq.getSeqNodeCompletenessMag());


  }
  
  /**
   * getGridDistFromFault
   */
  private void UpdateGridDistFromFault(){
	  double[] gridDist = DistDecayFromRupCalc.getRupDistList();
	  sequenceModel.setGridDistanceFromFault(gridDist);
  }
