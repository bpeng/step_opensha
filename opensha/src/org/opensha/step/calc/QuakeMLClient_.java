...
public class QuakeMLClient_ {
	...
	private static Logger logger = Logger.getLogger(QuakeMLClient_.class);
	...
	public ObsEqkRupList retrieveEvents(Calendar startDate, Calendar endDate){	
		...
		try {
			...
			if (statusCode != HttpStatus.SC_OK) {
				...
				JsonNode     jsonRoot   = mapper.readTree(new InputStreamReader((method.getResponseBodyAsStream())));				
				//logger.info("0 #### features " + jsonRoot.get("features").size());
				ArrayNode jsonFeatures = (ArrayNode) jsonRoot.get("features");
				...
				for(int i = 0; i < jsonFeatures.size(); i++){
					...
					//if(i < 100 ) logger.info("1 #### rupture\n " + rupture.getInfo());
